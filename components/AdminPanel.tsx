        ...
        <div className="p-8 space-y-8">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <h3 className="text-sm font-bold uppercase text-slate-500 mb-4 flex items-center gap-2">
                    <Globe size={16}/> Pengaturan Publik
                </h3>
                    ...
                    <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 border border-slate-300 rounded-lg px-3 py-2 focus-within:ring-2 focus-within:ring-blue-500 bg-slate-50">
                            ...
                            <input 
                                ...
                                placeholder="https://majene.bps.go.id/..." 
                            />
                        </div>
                    </div>
                    <button 
                        ...
                        className="bg-slate-900 text-white px-5 py-2.5 rounded-lg text-sm font-bold hover:bg-emerald-600 transition-colors disabled:opacity-50"
                    >
                        {isSavingSettings ? 'Menyimpan...' : 'Update Link'}
                    </button>
                </form>
            </div>
                                ...
                                <tr key={row.Id} className="hover:bg-slate-50 transition-colors group">
                                    ...
                                    <td className="p-5">
                                        <div className="flex items-center gap-2">
                                            {row.ShowOnHome && (
                                                <span title="Ditampilkan di Beranda">
                    ...
                    <form action={handleSave} className="p-6 space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-6">
                             ...
                             <div className="space-y-4">
                                ...
                                <div className="relative">
                                    ...
                                    <div className="relative">
                                        <input 
                                            ...
                                            onFocus={() => modalMode === 'add' && formId.length > 0 && setShowIdSuggestions(true)}
                                            className={`w-full border p-2.5 rounded-lg text-sm font-mono ${modalMode==='edit' ? 'bg-slate-100 text-slate-500' : 'bg-white focus:ring-2 focus:ring-blue-500'} outline-none`} 
                                                ...
                                                idSuggestions.map((item, idx) => (
                                                    ...
                                                    >
                                                        <div className="flex justify-between items-baseline mb-0.5">
                                                            <span className="text-xs font-bold text-emerald-600 font-mono bg-emerald-50 px-1.5 rounded">{item.var_id || item.id_variable || item.Id}</span>
                                                        </div>
                                                        <div className="text-xs text-slate-600 line-clamp-2 leading-relaxed">
                                                            {item['Nama Variabel'] || item.label || item.Label}
                                                        </div>
                                                    </div>
                                                ))
                                            ) : (
                                ...
                                <div>
                                    ...
                                    <input name="label" value={formLabel} onChange={e=>setFormLabel(e.target.value)} className="w-full border border-slate-300 p-2.5 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Label Indikator"/>
                                </div>
                                <div>
                             ...
                             <div className="space-y-4">
                                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-xs font-bold uppercase mb-1 text-slate-500">Grafik</label>
                                        ...
                                        <div>
                                            <label className="block text-xs font-bold uppercase mb-1 text-slate-500">Warna</label>
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold uppercase mb-1 text-slate-500">Logika Tren</label>
                        ...
                        <div className="p-4 bg-indigo-50 rounded-xl border border-indigo-100">
                             <h4 className="text-sm font-bold text-indigo-900 flex items-center gap-2 mb-4">
                                <Home size={14}/> Pengaturan Beranda & RPJMD
                             </h4>
                             <div className="grid grid-cols-2 gap-6">
                                <label className="flex items-center gap-3 cursor-pointer">
                                    <input 
                                        type="checkbox" 
                                        name="showOnHome" 
                                        checked={formShowHome} 
                                        onChange={e => setFormShowHome(e.target.checked)}
                                        className="w-5 h-5 rounded text-indigo-600 focus:ring-indigo-500 border-gray-300"
                                    />
                                    <span className="text-sm font-bold text-indigo-900">Tampilkan di Beranda</span>
                                </label>
                                
                                <div>
                                    <label className="block text-xs font-bold uppercase mb-1 text-indigo-800 flex items-center gap-1">
                                        <Target size={12}/> Target RPJMD
                                    </label>
                                    <input 
                                        name="targetRPJMD" 
                                        value={formTargetRPJMD} 
                                        onChange={e=>setFormTargetRPJMD(e.target.value)} 
                                        className="w-full border border-indigo-200 p-2 rounded-lg text-sm bg-white focus:ring-2 focus:ring-indigo-500 outline-none" 
